...
@RequiredArgsConstructor public class HistoryManager {
        ...
        sqliteManager.getPolyJDBC().transactionRunner().run(new VoidTransactionWrapper() {
            @Override public void performVoid(final QueryRunner queryRunner) {
                queryRunner.insert(sqliteManager.getPolyJDBC().query().insert().into("history")
                    ...
                    .value("user", message.getAuthor().getIdLong())
                    .value("message", message.getContentRaw())
    ...
    public List<String> getMessages(final long userId, final long channelId, final int count) {
        final List<String> strings =
            new ArrayList<>(sqliteManager.getPolyJDBC().transactionRunner().run(queryRunner -> {
                final SelectQuery selectQuery =
                    sqliteManager.getPolyJDBC().query().select("message").from("history")
                        .where("user = :user AND channel = :channel").limit(count)
