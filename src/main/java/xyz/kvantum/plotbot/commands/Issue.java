...
@CommandDeclaration(command = "issue", usage = "issue [id]", description = "Get a github issue")
@RequiredArgsConstructor public class Issue extends Command {
    ...
    @Override public boolean onCommand(CommandInstance instance) {
        if (instance.getArguments().length == 0) {
            instance.getCaller()
                .message("The issue tracker can be found at: " + BotConfig.Github.issueLink);
        } else {
            ...
            try {
                ...
                embedBuilder
                    .setTitle("Issue #" + issue.get().getId(), issue.get().getHtmlUrl().toString());
                embedBuilder.addField("Title", issue.get().getTitle(), true);
                embedBuilder
                    .addField("Opened", dateFormat.format(issue.get().getCreatedAt()), true);
                ...
                channel.getGuild().getTextChannelsByName(CmdIssue.channel, true).get(0)
